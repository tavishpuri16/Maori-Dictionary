...
@app.route('/login', methods=['GET',
                              'POST'])  # GET is when viewing something without changing, whereas POST is when changing something
def login():
    if is_logged_in():
    ...
    if request.method == "POST":
        ...
        email = request.form[
            'email'].strip()  # requests the right data from the form the user filled out - in this case the email
        ...
        try:
            ...
            return redirect(
                '/login?error=email+invalid+or+password+incorrect')  # if firstname, userid or db_password not found

        if not bcrypt.check_password_hash(db_password, password):
            return redirect(
                request.referrer + "?error=email+invalid+or+password+incorrect")  # if password does not match password in database

        if len(password) < 8:
            return redirect(
                '/signup?error=Password+must+be+8+characters+or+more')  # if password is less than 8 characters
...
def render_signup_page():
    ...
    if request.method == 'POST':
        ...
        if len(password) < 8:
            return redirect(
                '/signup?error=Password+must+be+8+characters+or+more')  # doesn't allow password of less than 8 characters
...
def render_add_word():
    ...
    if request.method == "POST":
        ...
        cur = con.cursor()
        try:
            cur.execute(query, (english, maori, userid, definition, level, image,
                                timestamp))  # the commas make the fields a tuple(takes words as opposed to characters)
        except sqlite3.IntegrityError:
...
def delete_word(words):
    ...
    con = create_connection(DB_NAME)
    cur = con.cursor()
    cur.execute("DELETE FROM words WHERE english=?",
                (words,))  # deletes the english word and associated fields from the dictionary
    con.commit()
    con.close()
    return redirect('/dictionary')
...
def render_word_page(xword):  # xword is the word that is being clicked on
    ...
    con = create_connection(DB_NAME)
    cur = con.cursor()
    cur.execute("SELECT english, maori, level, definition, image, timestamp, id, userid FROM words WHERE english=?",
                (xword,))  # executes query and passes the word being clicked on
